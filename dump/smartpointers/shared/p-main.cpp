...
int main() {
	...
	*smPtr = "hackett";
	...
	cout << boolalpha;
	cout << (charPtr == nullptr) << endl; // calls operator==

	  // actually, you'll never want to do this with a UniquePtr so we won't
	  // overwrite it for this one. But we will for SharedPtr, which can check
	  // if two SharedPtrs are pointing to the same thing.
	UniquePtr<char> charPtr2(new char('Z'),
		           [] (char* c) { delete c; });	
	// cout << (charPtr == charPtr2) << endl;
}